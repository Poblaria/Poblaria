export const HOUSES = [
    {
        id: "fake-5",
        latitude: 42.4436,
        longitude: 1.1344,
        title: "Traditional Stone House",
        price: "€350,000",
        image: "/images/villa_for_rent_in_spain_026-1.png",
        address: "Carrer Major, 15, 25597 Rialp, Lleida",
        description:
            "Charming stone house in the heart of Rialp with modern amenities and a cozy fireplace.",
        rooms: 3,
        bathrooms: 2,
        area: 180
    },
    {
        id: "fake-6",
        latitude: 42.444,
        longitude: 1.135,
        title: "Mountain View Villa",
        price: "€550,000",
        image: "/images/Torre-Caballe-Catalonia-Olivers-Travels1.jpg",
        address: "Carrer de la Font, 8, 25597 Rialp, Lleida",
        description:
            "Spacious villa offering stunning mountain views, a large garden, and a private pool.",
        rooms: 4,
        bathrooms: 3,
        area: 250
    }
];

export const JOBS = [
    {
        id: "fake-11",
        latitude: 42.4437,
        longitude: 1.1345,
        title: "Baker at La Fornal",
        salary: "€650,000",
        company: "La Fornal",
        address: "Carrer Major, 12, 25597 Rialp, Lleida",
        description:
            "Join our team as a skilled baker and create delicious pastries and bread in a traditional bakery setting."
    },
    {
        id: "fake-10",
        latitude: 42.4441,
        longitude: 1.1347,
        title: "Bartender at La Taverna",
        salary: "€550,000",
        company: "La Taverna",
        address: "Carrer de l'Església, 5, 25597 Rialp, Lleida",
        description:
            "Seeking an experienced bartender to craft cocktails and provide excellent customer service in a lively tavern atmosphere."
    }
];
