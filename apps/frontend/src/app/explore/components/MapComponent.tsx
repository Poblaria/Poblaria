...
function getViewForCountry(country: Country) {
  if (country === "FR") return { center: [46.6, 2.2] as [number, number], zoom: 6 };
...
export default function MapComponent({ dataType, housings, jobs, error }: MapComponentProps) {
  ...
  if (dataType === "houses" && !housings && !HOUSES.length) return <div>Loading houses...</div>;
  if (dataType === "jobs" && !jobs && !JOBS.length) return <div>Loading jobs...</div>;
      ...
      <Box sx={{ position: "relative" }}>
        ...
        >
          <Button
            ...
            sx={country === "ES" ? { bgcolor: "#5E7749", "&:hover": { bgcolor: "#83A16C" } } : {}}
            onClick={() => {
              setCountry("ES");
          ...
          <Button
            ...
            sx={country === "FR" ? { bgcolor: "#2D5B8A", "&:hover": { bgcolor: "#3C73AD" } } : {}}
            onClick={() => {
              setCountry("FR");
              setSelectedRegionName(null);
            }}
          ...
          <Button size="small" variant="text" onClick={() => setSelectedRegionName(null)} sx={{ ml: 1 }}>
            {t("home.regionSelector.reset")}
          </Button>

          {selectedRegionName && (
            <Typography variant="body2" sx={{ ml: 1 }}>
              {t("home.regionSelector.selected")} <b>{selectedRegionName}</b>
            </Typography>
          )}
        </Box>
        ...
        >
          ...
          <ZoomListener onZoomChange={(value) => setCurrentZoom(value)} />
