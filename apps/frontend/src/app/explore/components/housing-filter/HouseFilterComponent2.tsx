...
import { Box, Chip, IconButton, Paper, Stack, Typography, Button, Checkbox, FormControlLabel } from "@mui/material";
...
const SERVICES = ["Schools", "Health Center", "Local Market", "Public Transport", "Internet Access"];
...
export default function Step2KeepSearching({
  ...
  const toggleLifestyle = (label: string) => {
    setFilters((p) => ({
      ...p,
      lifestyle: p.lifestyle.includes(label) ? p.lifestyle.filter((x) => x !== label) : [...p.lifestyle, label],
    }));
  };
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 3 }}>
        ...
        <Typography sx={{ fontWeight: 800, fontSize: 22 }}>Keep searching for your best fit...</Typography>
      </Box>

      {/* Lifestyle */}
      <Box sx={{ mb: 4 }}>
        <Typography sx={{ fontWeight: 700, mb: 1 }}>Lifestyle</Typography>
      ...
      {/* Condition */}
      <Box sx={{ mb: 4 }}>
        <Typography sx={{ fontWeight: 700, mb: 1 }}>Condition</Typography>
        ...
        <Box sx={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <FormControlLabel
            control={
              <Checkbox
                ...
                onChange={() =>
                  setFilters((p) => ({ ...p, condition: p.condition === "MOVE_IN_READY" ? null : "MOVE_IN_READY" }))
                }
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={filters.condition === "NEEDS_RENOVATION"}
                onChange={() =>
                  setFilters((p) => ({
                    ...p,
                    condition: p.condition === "NEEDS_RENOVATION" ? null : "NEEDS_RENOVATION",
                  }))
                }
                sx={{ "&.Mui-checked": { color: "#83A16C" } }}
              />
      ...
      {/* Nearby Services */}
      <Box sx={{ mb: 5 }}>
        <Typography sx={{ fontWeight: 700, mb: 1 }}>Nearby Services</Typography>
